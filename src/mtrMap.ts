...
class MtrMap {
  ...
  constructor(options: MapOptions) {
    if (!L) {
      throw new Error('Please add leaflet and other necessary scripts to head tag.');
    }
  ...
  private renderAddressOn = async (marker: LatLng) => {
    ...
    if (status === 'OK') {
      ...
      if (this._options.inputs && data) {
        ...
        new Map(Object.entries(data?.subdivisions || {})).forEach(function (key: any, value: string) {
  ...
  private setInputs(values: any) {
    const { province, county, suburb, city, village, address } = this._options.inputs;
  ...
  private getLatLngBy = async (address: string): Promise<SearchByAddressResponse> => {
